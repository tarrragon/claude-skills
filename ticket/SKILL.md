...
| `track stuck-anas`  | 列出卡住的 ANA（in_progress 且落地路徑 spawned_tickets／children 全 completed，W17-008.15 / W3-815） | `ticket track stuck-anas --wave 3`         |
...
> 重複偵測：Tier 1 警告層（相似度 >= 0.3，不阻擋）+ Tier 2 阻擋層（同窗口高相似度 exit 1，`--allow-duplicate` 旁路）；見 `references/create-command.md`「重複偵測（兩層防護）」章節（W1-040.1 冪等防護）

> **主題歸屬（自動推導，0.2.1-W3-828）**：未指定 `--topic` / `--new-topic` 時，`create` 依三條判準自動推導主題，命中即自動指派並印出依據，建票者可否決後改派：
>
> | 判準 | 條件 | 成本 |
> |------|------|------|
...
>
> 三判準皆未命中時印 WARNING 但**不改 rc**（0.2.1-W3-829 過渡期 warn-only）：以 exit code 表達強制力會讓代理人誤判建票失敗而重試。要免除警告有兩條路——指定主題，或以 `--no-topic` 明示不指派（該旗標與 `--topic` / `--new-topic` 互斥，同給時於任何持久化前 exit 1）。
>
...
>   --category ticket-tracked --reason "W17-085 hook 訊息改善"
> ```
>
...
>
> **注意**：`depth <id>` 沿 `parent_id` 鏈計算嵌套深度（**非** ID 字串數點，避免完整版本前綴 `1.0.0-W1-056.5` 含 3 個點被誤算為 depth 4 的 linux F1 fatal bug）。輸出 `depth` / `max_depth`（= `MAX_TICKET_DEPTH=3`）/ `can_descend`（depth < MAX_TICKET_DEPTH）。深度定義：根任務（`parent_id: null`）= depth 1，每往下一層 +1。用途：agent 自檢層級自覺（協議 v2 D3），無需上層 prompt 傳遞層級資訊。用法：`ticket track depth <ticket-id>`。
>
> **注意**：`create --parent <id>` 時，若新子任務深度 >= `MAX_TICKET_DEPTH`（3）會 emit warning（**不硬擋**，留旁路）。深度同樣沿 parent_id 鏈計算（W1-056.5 協議 v2 D3）。此為嵌套派發深度上限的 CLI 強制層，使協議深度上限不只是文件建議。
>
...
- v2.13.0 (2026-08-20): `create` 主題自動推導上線（0.2.1-W3-828 落地）
...
- v2.12.0 (2026-08-20): `board` 新增 `--group-by {wave,topic}` 分組軸（0.2.1-W3-805 落地）
