...
## `command not found` 分診：沒裝 vs 裝了不在 PATH

`command not found` 有三個不同根因，修法完全不同——先分診再動手，不要一律 `install`：

```bash
find / -name '<tool>' -type f 2>/dev/null   # binary 到底在不在機器上
echo "$PATH" | tr ':' '\n'                  # 現在的 PATH 有哪些目錄
```

- **真的沒裝**：`find` 找不到 binary → 裝（注意這發行版可能叫別的名字，見第零步）。
- **裝了、但不在 PATH**：`find` 找得到（如 `/opt/<tool>/bin` 或 `~/.local/bin`），但那個目錄不在 `$PATH`。這常見於安裝器把 PATH 設定 append 進某個 profile（`~/.bashrc` / `~/.zprofile`）或系統 drop-in（`/etc/profile.d/`），而目前這個 shell 沒載入那個 profile。修法是把該目錄補進 PATH 的宣告來源，不是重裝。
- **互動 shell 能用、script 裡不能用**：這是 PATH 作用域問題。互動 shell 載入了 `.bashrc` / `.zshrc`（裡面有安裝器 append 的 PATH），但非互動的 script shell 沒載入那些 init 檔，於是同一個工具在 terminal 打得動、在 `install.sh` 裡 `command not found`。另一種同型：script A 呼叫子 script B、B 裡 `export PATH` 或 `eval shellenv` 改的 PATH 回到 A 就沒了（子行程改不了父行程）。修法是在需要它的那個作用域（script 本身、或父 orchestrator）明確補 PATH，不能假設互動 shell 的 PATH 會傳進來。

分診口訣：`find` 決定「沒裝 vs 裝了」，`$PATH` 對照決定「哪個目錄漏了」，互動 vs script 決定「哪個作用域沒拿到」。
