...
- **A: 寫作規範 audit** — 專案寫作規範（如 AGENTS.md / markdown-writing-spec）/ compositional-writing 規範遵循
  - **字句層 grep（必跑）**——**下面是摘要、不是完整清單**。完整的一份在 `compositional-writing` 的「字句層 keyword bank（完整清單）」節，目前十七類；這裡沒有列到的至少有澄清式框架、歸因語氣、宣導語氣、脈絡懸置（等下再討論 / 前面兩個小節 / 用一句話總結 / 列舉鳥瞰）四類。派 reviewer 時把它導去那一節逐類跑，不要只跑這裡列到的：
    ...
    - 物理化錯配：`rg "撐得住|撐不住|撐得起|撐不起|能撐|撐住|撐起|撐不久|掛在|扛|垮|頂得住|咬得|咬合|啃|咬得|咬合|啃" <files>` — 同一張卡四種錯配裡唯一有一組穩定關鍵詞可掃的一種。抽象概念（證據 / 論證 / 結論 / 判準 / 方法 / 責任）配上承重、支撐、懸掛類動詞；判準可機械執行——問這個動詞照字面成立需要什麼物理條件，需要重量或支點的就是借來的。替換：足以支持 / 依賴 / 佐證 / 維持不久 / 負責。這一種的危害多一層：「撐得住」聽起來已經像答案，於是「支持什麼、到什麼範圍」不必被回答。例外是就地展開成可逐項對應的類比、以及主詞本來就是實體或系統負載
