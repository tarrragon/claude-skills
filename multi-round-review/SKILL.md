...
7. **要質疑一個 finding 時，要它交出物件、不要它交出判斷**（per [要交出物件、不要交出關於物件的判斷](references/principles/ask-for-the-object-not-the-verdict.md)）。關於物件的斷言（「這兩條判準不可同時滿足」「這個檔超標」）可以在抽象層完全合理，而**推翻它要離開抽象層、構造出那個物件**——回報者的主張在他自己的推理裡自洽，沒有人要求構造時他沒有理由去做。追問的動詞用「寫出來 / 算出來 / 構造一個」而不用「確認 / 說明 / 檢查」，並加一句「我要能自己驗證」（它把產物的接收者換成追問者，交出來的東西因此必須獨立於回報者的推理而可檢驗）。回報者在更早輪次量過的數字要明確要求代入並印出計算——實測一次，那個值一直在它手上而本輪的判斷沒有用到它。判別問句：這個宣稱要成立，需要一個什麼樣的物件（一個字串、一個數字、一組走得完的步驟）。**還沒起疑、只是想確認的時候換另一個施力點**：問一個只有握著原始輸出才答得出來的**後續問題**（那次問的是「這兩個機制你分不分得開」），回報者為了回答就得回去看，而一回去看就發現那份輸出不存在——「你確定嗎」得到的是同一個斷言再說一次，而第二次與第一次在文字上無法區分。查核責任的分類要**四欄**：引用的（有外部來源）／我量測的（工具輸出在對話紀錄裡、可原地重跑）／我推導的（沒來源、引用前重算）／我以為觀測到的（那次呼叫從未發生，撤回）。**第二欄的定義是分界線——在不在對話紀錄裡可以驗，「我記得看過」不能**，而兩欄的分類法會把第四類歸進第一類，因為它看起來有來源。詳見 [「我以為觀測到的」躲在「引用的」那一欄裡](references/principles/fabricated-observation-hides-among-cited-facts.md)
