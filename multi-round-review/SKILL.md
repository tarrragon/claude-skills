...
這一段排在工具清單之前，因為產出層級決定這一批要做多少事——每個 frame 跑幾次、改動分幾批驗、要不要保留逐輪的材料。歸因要單變數實驗，而單變數實驗的定義就是一次只准動一個變數——於是 N 次改動要 N 次全量驗證。**審查對象是累積式產物（題庫、案例庫、逐輪讀數）時這件事沒有收斂點**：驗證成本由既有存量決定而每次改動的價值固定，分母一直長而分子不動。一次實測的下場是單一工作階段新建 82 張票、關閉 35 張。**每一張票單獨看都是真缺陷，所以「把判斷標準訂嚴一點」擋不住它**——發散來自問題選錯，不是來自誤報。
...
| 2-A Cadence                            | 一批三篇以上                                         | 單篇不適用（同骨化是跨篇現象）；規範指定的位置要另外普查段名，「這個重複是規範要求的」是加強掃描的訊號而不是豁免理由                        |
...
- **零 finding 分三格**：掃過有候選判合規／判斷標準對此對象結構上不適用／沒掃
- **自評欄與比對欄分開**：自報「我掃了」與比對「掃到了什麼」是兩欄
- **結案後的改判登記進下一輪輸入**，不回頭補進已結案的清單
- **嚴重度按讀者學不學得會分級**：必修＝照做會學錯做錯、在這一段理解不了、要離頁或回頭翻才讀得懂、作者編排脈絡寫進正文；可改＝拿得到也不會錯但多花力氣。**不寫「寧可少報」「措辭偏好不回報」**——這兩條讓審查者判斷的是有沒有缺資訊，而宣稱過寬、未教先用、隔段指稱都不缺資訊
