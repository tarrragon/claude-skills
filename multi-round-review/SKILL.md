...
- **零 finding 怎麼分類**（要求揭露得到一鍋端、要求分類才得到分層）：寫「這一維跑過、零命中」擋掉了省略，而它擋不住把證據力不同的東西壓成同一句——**一鍋端比省略難察覺，因為它已經照契約寫了**。契約要給定類別、由 reviewer 逐條歸位，三格窮盡：**造過候選情境、判定不成立**（是證據）／**判準只有二元事實、造不出情境**（不適用本 frame，不可當證據）／**沒查**（未知）。少了第三格，沒查的會被歸進最接近的一格，而最接近的是第一格。實測一次：六十六條判準回報三十四條零命中、格式完全合規，追問採信依據後拆成 17 / 16 / 0——**分類是被問句逼出來的，報告者本來只有一堆**，所以欄位動詞寫「說明」會退回原本的粒度，要寫成填空。詳見 [要求揭露只擋得住省略](references/principles/disclosure-yields-one-bucket-classification-yields-layers.md)
