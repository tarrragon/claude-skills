...
| **二元處置取捨**                                     | 準備向使用者呈現「保留 / 回退 / 停用 / 刪除」類處置選項，或候選方案在不同維度上代價方向相反（例如強度高者風險亦高）的設計選型選項，而該清單未經 W 階段產出 | **W 階段起跳**（反向框架 + 三類涵蓋跑完才列選項）；處置涉及防護能力增減或不可逆變更則升為完整 |

> 上述 4 項決策路徑層因子可由各專案映射到自己的掛鉤（Hook）、CLI 或任務系統；本 skill 只保留通用判斷語意。

### 二元處置取捨：與既有結論錨定（3.2）的邊界

兩者都處理選項空間過窄，觸發時機不同：**既有結論錨定（3.2）在 WRAP 內部**——W 階段已跑，發現產出的選項能被一句話概括；**二元處置取捨在 WRAP 之前**——尚未進入 WRAP，直覺已浮出一組對立選項並準備直接呈現。3.2 是自我檢查，本項是入場閘門。

**適用對象不限處置類**：除「保留 / 回退 / 停用 / 刪除」類處置選項外，設計選型選項（如「接線方式該選哪種形態」）若在不同維度上代價方向相反（例如強度高者風險也高、無旁路者防護力強但副作用風險也高），亦屬本項涵蓋範圍。兩者共享同一失效機制：選項清單由直覺產生、未經 W 階段擴增即呈現，使解空間在決策者不可見的情況下被封閉；差別僅在對象是「既有物的去留」還是「未建物的形態」。
...
**Action**：識別到自己正要列處置類選項，或候選方案在不同維度上代價方向相反的設計選型選項時停手，先跑 W 階段的反向框架檢查（原框架的反面是什麼？）與三類涵蓋（改造既有 / 零工具 / 新增工具），選項清單取自 W 階段產出。若 W 階段的產出與直覺清單一致，照列即可——最低成本是一次檢查，不是一次重寫。

**升級條件**：處置對象涉及防護能力增減（移除守衛、降級閘門、停用檢查）或不可逆變更時，W 階段不足，須跑完整模式。理由是這類處置的錯誤代價不對稱——R 階段的基本率與既有機制查證常會推翻 W 階段看似完整的選項集，而 A 階段的悖論檢查是「守衛是否誠實」這類問題的唯一結構化把關。
