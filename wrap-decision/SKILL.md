...
| **二元處置取捨**                                     | 準備向使用者呈現「保留 / 回退 / 停用 / 刪除」類處置選項，而該清單未經 W 階段產出 | **W 階段起跳**（反向框架 + 三類涵蓋跑完才列選項）；處置涉及防護能力增減或不可逆變更則升為完整 |
...
**Why**：處置類問題（「這個東西該留還是該拿掉」）天然以對立軸呈現，直覺產生的選項清單會被「該多硬 / 該留多少」這種程度軸限定，而真正的解常在程度軸之外（移位、換執行點、重新定義對象）。選項一旦列出即封閉解空間，且該封閉對決策者不可見——決策者只會看到自己列了三個選項，不會看到沒被列出的第四個。

**Consequence**：使用者只能在被限定的清單內選擇，或必須自己看穿框架才能提出第四個選項。決策品質上限由列選項者的框架決定，而非由問題本身決定。

**Action**：識別到自己正要列處置類選項時停手，先跑 W 階段的反向框架檢查（原框架的反面是什麼？）與三類涵蓋（改造既有 / 零工具 / 新增工具），選項清單取自 W 階段產出。若 W 階段的產出與直覺清單一致，照列即可——最低成本是一次檢查，不是一次重寫。
