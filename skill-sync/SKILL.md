...
- `push` runs a portability gate before cloning. A skill whose `SKILL.md` frontmatter declares `metadata.portable: true` is aborted when its markdown names another project's files (any `.claude/...` path) or carries a project ticket ID (`x.y.z-Wn-nnn`); the violating file, line and matched text are printed, and `--force` pushes anyway. A skill without that declaration only gets a one-line count on stderr — framework-bound tools (ticket, doc, worktree) talk about `.claude/` paths as their actual subject matter, and a strict default would freeze them out of sync entirely. Consumer-specific passages belong in `references/project-integration/`, which the gate does not scan because sync excludes it
