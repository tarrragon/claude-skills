...
**stub 盲區與語意級假後端**：bug 的成因是「我們對外部服務行為的假設錯誤」時，由測試餵資料的 stub 從結構上驗證不出來——stub 回放的就是那個假設、假設與斷言出自同一人之手，測試必然綠。補位是**有狀態的語意級假後端**：持有狀態、模擬已被證實的外部行為（不是想像的行為），讓多個元件走完整互動鏈的流程測試；並與週期性的真實後端驗證測試配對，讓外部行為漂移有地方現形。判讀訊號：整合測試的「後端」只是逐請求回放預錄回應、沒有自己的狀態機——它驗的是自己的假設。
