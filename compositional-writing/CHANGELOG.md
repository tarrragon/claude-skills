...
**Version**: 1.9.1 — 清掉四處借用消費端命名空間的 ticket ID 示例（`designing-fields.md` 兩處、`designing-fields-ticket-6w.md` 兩處），`W03-021` 改為 `wave03-021`。它們是教學用的格式示例而非真實票號，但字面命中 portability 閘門的 `\bW\d+-\d+\b`，於是每一次推送都要靠 `--force` 旁路才過得去。換掉命名空間之後那兩個 `portability-allow` 行內豁免也不必存在，一併移除——**豁免的存在讓真違規與正當引用在閘門輸出裡長得一樣**，而這支 skill 的兩個豁免正好都是可以直接改掉的那一種。示例的教學點未動（版本／wave／序號／子序號可用分隔符拆解），`wave03` 比 `W03` 更直接說出中間那一段是什麼。
