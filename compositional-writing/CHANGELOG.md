...
**Version**: 1.8.0 — 索引建立段新增「錨點標籤的自足義務隨引用點數上升」與同名 principle 卡。它限定 `sentence-self-sufficiency-by-consumption-unit` 檢索鍵位那一列的「精簡優先」：那條優先序的前提是這個鍵只有一個消費點，而鍵被別處逐字引用時它同時是錨點——引用是逐字比對，加字即斷開，所以引用點只交付短名本身，且既有標籤要改時修復成本同樣乘以引用點數。該卡給義務的名字（識別充足），本次補上可執行的判準：這個標籤裡的每個名詞讀者知道是哪一個嗎、每個動詞讀者知道主詞是誰嗎。**不進字句層 keyword bank**，因為這一類沒有穩定關鍵詞而且周圍都是對的——壓縮標籤讀起來是完整的詞，`compressed-chinese-terms-need-head-noun` 的四道殘片檢查全數放行；可機械執行的入口是引用點數（`rg -F -l` 命中兩檔以上），不是詞形。兩個實例：一則由併行 session 轉來（互動規格的表格列名壓成四個字、另被下游四處逐字引用），一則自測（`positioning-decides-form-before-any-rule-applies` 的七種裝置有四個引用點，三處保留完整形、一處砍掉限定語十九個字，而那一處正是逐項執行的檢查清單）。
