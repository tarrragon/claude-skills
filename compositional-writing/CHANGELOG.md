...
**Version**: 1.31.4 — `teaching-article-context` 依第三輪審查（誤用梯度與個案實跑）補齊規則：形態加上預設位置（正文談到那個可動項的那一句，接在結論之後、不插進推導中間）並改成逐條連結決定；第一種與第四種的觸發條件從形狀與「值得」改成可查的條件；不在同層的連結改成三類正面判定（往回、往前、層級），拿掉「表達不成換掉哪一項就不綁」這個排除式；「不能整批套同一種新形態」寫出判定單位（骨架三項），範圍擴到新寫的篇加入已處置的分類。

**Version**: 1.31.3 — `teaching-article-context` 依第二輪審查把四種形態從「條件 → 放在哪裡」的放置指令改寫成各自的理由（為什麼那個位置適合那種形態），避免教學定位的原則卡被讀成操作手冊。

**Version**: 1.31.2 — `teaching-article-context` 依審查修兩處：量詞範圍從「同一段話重複」改成「同一個段名與句型重複」；表格形態的條件從「各鄰居各改一項」改成「各鄰居各從它改掉一部分」（原句取自的實例本身每一道都改了不只一項）。

**Version**: 1.31.1 — `teaching-article-context` 補一段：第三節規定的是連結綁在什麼上、不規定形態，附四種由可動項位置決定的形態，以及「修一批已同形的末節時不能整批套同一種新形態」。成因是兩個分類把這一條做成同一個段名與同一組粗體句型收尾，逐篇合規而連讀是同一段話重複十幾次。
...
**Version**: 1.28.0 — 新增 `references/demonstrating-commands.md`（情境 5f：示範指令）。既有規範管的是排版的機械規則（語言標示、前後空行），而區塊裡放什麼沒有住址，缺口的形狀是每次生成各自發揮。這份定三種形態並給判準問句——讀者在這一節要做的是照著跑、選一種、還是查一個——並涵蓋：註解在渲染後與指令分屬兩層（高亮給註解另一種顏色），所以註解裝標籤、推導寫散文；上方註解與行尾註解的分工與長度上限；開了行號時空行也佔號，所以散文不用行號指涉區塊內容；並列對照為什麼最容易漏（兩種都正確、讀者要選的那一類）；參數表的觸發條件（三個以上選項且讀者逐列查）與用途欄深度隨判斷難度變化、不為整齊拉成等長；佔位符與字面值的分界，含「可以換而這一篇選了一個」這個邊界情形；三類不進區塊的東西（錯誤用法、shell 提示符、推導）與預期輸出的處理。附一次 before / after 走查與八項檢查清單。SKILL.md 的觸發路由表、檔案結構圖與〈示範指令的引數〉一節，以及 `writing-articles.md`、`writing-documents.md` 各加一條路由。
...
**Version**: 1.24.0 — 句子層文法標準加入核心原則（#357/#358 獨立於教室形態、適用全部書面產出）
...
**Version**: 1.20.0 — 字句層 bank 的〈掃描指令本身也要驗〉補正規化的兩個相反失效方向：換行與儲存格分隔符要**替換成單一空白**，不刪除也不當切段邊界——拿來切段會讓跨行跨格的命中漏報，直接刪除會把相鄰的字接成原文沒有的字串而生假命中，兩者都不報錯。取前後脈絡的掃描因此寫成 `.{0,20}` 而不是 `.{20}`，並在回報零行時先看計數是不是也為零——**計數非零而脈絡零行，零的是切段不是內容**。本條由併行專案的跨 repo 對照提供機制，而本站同一天至少四次因為行首位置而脈絡掃描回零、差點照零輸出判定。

**Version**: 1.19.0 — 新增 principle 卡 `derived-operational-form-needs-its-own-check`（從原則卡壓出來的操作形要獨立驗一次，而本體正確正是它不被檢查的原因——壓縮的人剛寫完那張卡、審查的人也讀過它，兩者讀衍生形時都自動把卡上的內容補回去，補完發生在讀者側且無聲；三種掉落各有實測：極性疊加、定義掉落、先行詞掉落）。`reference-authoring-standards.md` 新增〈從原則卡壓出來的條目：壓縮後三問〉一節（極性層數／術語在同節有無定義／指涉詞先行詞在同節有無出現，三問都不准往卡裡找答案，並明寫連結不算定義）。本節自己按那三問寫，是 1.18.0 那一輪探針的直接產物——該輪十一項回報有七項落在衍生形上而本體一項沒中。

**Version**: 1.18.0 — 1.17.0 新增的兩段各跑一批理解探針（每批三份、指令與餵入範圍逐字相同、不給審查框架、明令標記不確定），兩批合計十一項回報，逐項回原文歸因後七項進處置。**`reference-authoring-standards.md` 的〈條文的可觸發性與效力方向〉**：第三項 **3/3 讀成相反的意思**——原寫「禁止某條推理捷徑的句子沒有交代那條捷徑需要的前提（不宣告某組結論的個數、互斥性、窮盡性）」，三份全部讀成「必須宣告那三項」，照它執行會做出與規則相反的動作；成因指認得出來——同節第一、二項也以「沒有」起手而 3/3 讀對，差別是第三項把 `禁止` + `沒有` + `交代` 三個帶極性的詞疊在一句，已改為單層的「條文沒有宣告……」並與前兩項同形。**壞的是壓縮後的衍生形而非本體**：`rule-text-can-have-negative-effect` 卡上的原句（「不要在條文裡宣告某組結論的個數、互斥性或窮盡性」）清楚，缺陷發生在壓成 checklist 的那一步，而本站的字句層 bank 對它回報零命中。同節另修三處：擋回流那一句補主詞與依據（1/3 讀成「自認成立就可以往下走」）、「有一類的處置是刪除」指名是哪一類（3/3 問）、「拿掉結構說明之後」寫出它與上一項的關係（3/3 標歧義）。**改完以逐字相同的指令複驗三份，第三項 3/3 讀對**（單變項：只有稿件變了）；複驗掀出原本被那個大錯蓋住的一層——`正面觸發條件`（1/3 明報「無法確定判斷通過或不通過的標準，因為定義沒寫」）、`極性詞`（1/3 沒定義、1/3 擴張到「滿足」「違反」）、`雙重否定`（1/3 讀成「不是 X 而是 Y」，那是否定起手）、`零引用` 的作用域（3/3 問）四個承重術語在該節內沒有定義，卡上才有，已就地補上定義與反例。**`designing-fields.md` 的〈值域涵蓋檢查〉**：主旨與步驟 3/3 讀對、無極性反轉；兩項進處置——「那個數字」的先行詞在壓縮時掉了（卡上有「這一輪修了幾處」，該節沒有，3/3 各給一個不同的模糊指涉），以及兩類結論的分辨 **2/3 退化成關鍵詞比對**（「看有沒有『其餘』『只有』」）而機制句只在卡上、表格的兩個例子恰好都含那些量詞，已補機制句（真假由具體對象決定還是由集合劃得對不對決定）與「不要用量詞當判準」。另三項判為第三類（文章已給足材料排除該讀法）未進處置：補值不補說明的理由、沒有格子的結果是否為刻意宣告、分類的實測是否可推廣。
...
**Version**: 1.15.0 — 本 skill 自己跑了一次探針審查（三批九份、初測與複驗各一輪），依結果改。

翻轉的四項：〈這份檔案是綱領層〉的「原則本身不指定動作，那是設計、不是缺漏」改寫成正向陳述動作住在 reference 裡（3/3 讀成在為缺漏辯護 → 0/3）；原子化原則的交界段壓縮成主張加一句理由加指路、六條檢查的細節留在 `structuring-with-solid.md`（1/3 點名想跳過整段 → 0/3）；`structuring-with-solid.md` 補〈走完之後〉（下一步 3/3 說沒說 → 0/3）與「原子化原則」的位置（3/3 → 0/3）；同檔「受管詞」首次用點就地定義。

**達門檻而本輪未處置的一項要記住**：核心原則段偏長，初測 3/3、複驗 3/3，而本輪只做了一次壓縮。依換手段的條件，下一個手段是按單一責任把 SKILL.md 拆開，不是再壓縮一次。

**Version**: 1.14.0 — 內容壓力原則補下界。「把論證外移、只留主張句」是合法的壓縮方向，而它有下界：**主張句旁邊要留得住「為什麼」的那一句**。一次冷讀在依此壓縮過的稿件上直指「這是對做法的否定，但沒立即說明為什麼」——**沒有理由的禁令讀者記不住，也判不出邊界**，因為邊界是從理由推出來的，理由不在場時讀者只有照字面套用到不該套的地方或整條忽略兩條路，而那句禁令在版面上看起來仍然寫著。SKILL.md 原子化段與 `content-pressure-resolves-by-expansion-not-compression` principle 卡各補一段。

**Version**: 1.13.0 — 新增拆分之後的交界檢查，來源是一次「單檔拆成八檔」之後的三輪冷讀量測（二十八份低階探針、七項判決，而該份文件在量測前已通過三輪高階 reviewer 的九個 frame、七項無一被攔下）。

`structuring-with-solid.md` 新增〈拆完之後的交界檢查〉一節：前面五條 SOLID 映射決定該不該拆、沿哪條軸拆，這一節的對象是**拆分自己造出來的東西**——檔與檔之間的交界，而**每一份單獨讀都是對的、逐檔的自審與逐檔的冷讀對它恆真**。六條逐項檢查（兄弟檔互指對稱、位置座標寫上一步下一步不寫總數、層級不得混疊、用詞定義與使用同檔、內嵌計數改分組名、受管詞指路在首次用點），三個子段展開最不直觀的三項：**切法會決定層級混疊會不會發生**（依角色切讓每一份從自己的第一步編號、同一件事在兩個層級各編一次號，依步驟切不產生它，所以流程型內容的拆分軸優先選步驟）、**受管詞的指路要在首次用點**（單變項實測：八份檔頭都有總括式指路而三份仍判「沒說去哪查」，只把一個詞改成首次用點就地帶指路之後降為零份，結論是讀者在正文遇到那個詞時不會回頭查檔頭）、**外移之後留在原處的殘餘要自足**（拿掉被讀成掏空、補回三項後果被讀成堆砌，自足的成本是一個具體實例）。同檔的觸發表、適用聲明、結構檢查清單（加三列交界訊號）與檢查時機（交界組在拆分完成的當下跑、受測單位是整組檔案）一併更新，該 reference 升 1.1.0。

`SKILL.md` 原子化原則新增對應段落、觸發路由補「剛把一份文件拆成多檔要查交界」。新增 principle 卡 `split-defects-land-between-the-files`。

**Version**: 1.12.0 — 讀者定位那一段之後新增反向配對「不寫會取代讀者查證的東西」與同名 principle 卡。特定實例的值與處理方式（某台機器、某個環境、某次安裝）寫進文字之後就佔據了讀者原本要自己查的位置——他要的是「我這一台是什麼」而文字給了「有一台是什麼」，而特定實例的值正好是他那一台會不一樣的那些。**危害與正確性無關**：值可以完全正確、來源完全可靠，而它從來就不是關於他的系統的。判別問句：讀者拿這一句去用，他是去查自己的系統，還是直接照抄。**「標註未驗證」對這一類不夠**，差別在誰承擔預設——標註要讀者的自律才生效，不寫由結構保證；而照抄的人多半搜尋直達那一行、複製、離開，路徑上不經過那個標記。伴生判斷「需要實機才寫得了」多半是把「有了會更好」講成「需要」。**與前一條放在同一段是刻意的**：兩者是反向的一對（該寫而沒寫 / 不該寫而寫了），而兩種缺陷都在稿件上看不出來，只讀到其中一條的人會用它去合理化另一條的違規。

**Version**: 1.11.0 — 讀者定位那一條補上它的限定，`audience-is-professional-not-layperson` 同步。**這條定位是能力宣稱、不是知識宣稱**——它說讀者有能力理解（研究方法、開發原則、邏輯推導、開發經驗），不說他已經知道這個領域的名詞、原理與縮寫；兩種讀法對同一句話都通順，而只有後一種會授權省略基本名詞、原理與知識卡。方向由需求決定：專業讀者要的是每個流程的原因、成本、代價與用詞精準（`2*4` 與 `1*8` 在執行中是兩回事），全部是更多的文字，所以這條原則按它描述的需求生效時推向展開。判別問句：這一處省略，省下的是讀者不需要的字，還是讀者要自己補回來的脈絡。偵測靠術語分級表與理解取樣、不靠讀稿——省略在產物上沒有痕跡（沒補的卡不存在、沒展開的原理不佔行數），以稿件為輸入的檢查全盲。**限定與本體同檔是刻意的**：這條規則的失效模式正是有人只讀到半條，而擋住「把讀者當外行人」的那句話同時是「讀者不需要這個解釋」最有力的依據，拆檔會在載入時重演同一個風險。

**Version**: 1.10.2 — 1.10.1 的紀錄行補上 `rule8-exempt` 標記。**修法的紀錄必須寫出被修掉的那個字面**，於是 1.10.1 說明 ticket ID 汰除時重新引入了它，裝有引用穩定性守衛的消費端因此拉了 1.10.1 就提交不了（該守衛不豁免行內程式碼片段，與同專案的禁用詞掃描不同源）。由該消費端回報並提供標記文字。

這個形態與 1.10.1 更正的那件事同型而載體不同：那次是修覆蓋的動作本身造成覆蓋，這次是修違規的紀錄本身帶著違規。**汰除某個字面時，記錄那次汰除的文字是它最後一個合法住址**，而任何以字面為判準的檢查對這個住址與對真違規一視同仁。

**Version**: 1.10.1 — 更正 1.10.0 並復原被它蓋掉的 1.9.1 條目。**1.10.0 的敘述有兩處與事實不符**：它寫「本方的段落第二次消失」而當時對方已在 1.9.1 自行復原（做法是自發佈庫 git 歷史取回 1.9.0 全文為基底重新套用），也寫「原文指的是兩條路徑」而對方在同一版已把它改成指名身分。兩處誤判的共同來源是**拿 `raw.githubusercontent.com` 當發佈庫的權威狀態**——那個端點有 CDN 快取，回讀到的是數分鐘前的內容，而它與真實狀態的落差不會有任何訊號（讀得到、格式正確、看起來就是答案）。查發佈庫要 `git clone`，或讀 skill-sync 自己印的 `[Version] local X vs remote Y`（它比對的是 clone，不是 CDN）。

<!-- rule8-exempt: illustration:本條記錄的正是該 ID 形態的汰除，說明對象即 ID 本身 -->
後果是 1.10.0 成為一次多餘的重寫，並蓋掉對方 1.9.1 那條含根因分析與預防規則的紀錄——**與它想修的錯誤同型**。本版把 1-A 那一則改回對方 1.9.1 的措辭（該版先發佈，且它指名了正本的節名，比本方的改寫更精確）、把對方的 1.9.1 條目原文放回。1.10.0 實際帶進來且保留的唯一有效變更是四處 ticket ID 命名空間清理（`W03-021` → `wave03-021`，`designing-fields.md` 與 `designing-fields-ticket-6w.md` 各兩處，同時移除兩個因此不再需要的 `portability-allow` 行內豁免）——它原記為 1.9.1、與對方的 1.9.1 同號，改記於此。

**Version**: 1.10.0 — 合併發佈庫的 1.8.1（本方 1.9.0 推上之後、由未持有 1.9.0 的一方從 1.8.0 出發推出，本方的段落與兩條 CHANGELOG 條目第二次消失，且版號同時倒退）。**本次的處置改為推完立刻回讀發佈庫驗證**，前一次的覆蓋是靠下一輪推送前的版號比對才發現的。

1.8.1 的方向採納：「1-A 掃描的另兩份判準」那一則不再複述條件內容、改為指向正本。其寫法改一處——原文指的是兩條 `.claude/rules/core/` 路徑且無 `portability-allow` 標記，在宣告 `portable: true` 的 skill 裡是 consumer-path 違規（它會讓下一個推送的人又需要 `--force`，而那正是 1.9.1 剛清掉的狀態）。改為指名文件身分而不寫路徑，符合「外部引用指名身分、不寫路徑」，不複述條文這個改進完整保留。

**Version**: 1.9.1 — 「1-A 掃描的另兩份判準（非本清單成員）」一則改寫為指向句：隱含詞、禁用詞豁免、zhtw cross_strait／confusable 三條邊界已補進其正本（`document-writing-style.md`〈撰寫前檢查清單〉、`language-constraints.md` 規則 2），本節不再重複載述條文內容，只指向正本。

**本條為復原後重推**：該修正原以 1.8.1 推出，而發佈庫當時已在 1.9.0——推送端帶 `--force` 略過方向警告，把 1.9.0 覆蓋為 1.8.1。復原方式為自發佈庫 git 歷史取回 1.9.0 全文為基底、重新套用本條修正、讓號至 1.9.1。**同一推送端在 1.8.0 已犯過一次同型錯誤**（與並行 session 的 1.8.0 同號相撞、覆蓋對方內容，由對方合併兩側後讓號至 1.9.0），兩次的共同成因是 `--force` 把「版號比對」與「三方方向判讀」兩道警告一起略過，而執行者只讀了推送成功那一行。**避免方式**：推送前先跑不帶 `--force` 的 `skill-sync push`，讀完 `[Version] local X vs remote Y` 那一行再決定；本地版號不高於遠端時一律先 pull 合併。

**Version**: 1.9.0 — 索引建立段新增「錨點標籤的自足義務隨引用點數上升」與同名 principle 卡。**這一條原本推成 1.8.0，與另一個並行 session 的 1.8.0 版號相撞而在發佈庫被覆蓋**（兩邊改的區域不相交：對方在字句層 keyword bank 六條後面各加合規邊界、本方在索引建立段加一段，覆蓋是整檔推送造成的，不是內容衝突）。principle 卡當時留在發佈庫成為無人引用的孤兒，本次把入口補回並改號到 1.9.0——對方的 1.8.0 已發佈、其他消費端可能已拉取，所以讓號的是後發現的一方。

內容：它限定 `sentence-self-sufficiency-by-consumption-unit` 檢索鍵位那一列的「精簡優先」——那條優先序的前提是這個鍵只有一個消費點，而鍵被別處逐字引用時它同時是錨點，引用是逐字比對、加字即斷開，所以引用點只交付短名本身，且既有標籤要改時修復成本同樣乘以引用點數。該卡給義務的名字（識別充足），本次補上可執行的判準：這個標籤裡的每個名詞讀者知道是哪一個嗎、每個動詞讀者知道主詞是誰嗎。**不進字句層 keyword bank**，因為這一類沒有穩定關鍵詞而且周圍都是對的——壓縮標籤讀起來是完整的詞，`compressed-chinese-terms-need-head-noun` 的四道殘片檢查全數放行；可機械執行的入口是引用點數（`rg -F -l` 命中兩檔以上），不是詞形。兩個實例：一則由併行 session 轉來（互動規格的表格列名壓成四個字、另被下游四處逐字引用），一則自測（`positioning-decides-form-before-any-rule-applies` 的七種裝置有四個引用點，三處保留完整形、一處砍掉限定語十九個字，而那一處正是逐項執行的檢查清單）。

**Version**: 1.8.1 — 「1-A 掃描的另兩份判準（非本清單成員）」一則改寫為指向句：隱含詞、禁用詞豁免、zhtw cross_strait／confusable 三條邊界已補進其正本（`document-writing-style.md`〈撰寫前檢查清單〉、`language-constraints.md` 規則 2），本節不再重複載述條文內容，只指向正本。

**Version**: 1.8.0 — 一次跨 session 異源交換掃描（另一個並行 session 拿本檔判準掃同批稿件、本方拿對方判準互掃）寫明的合規邊界補進字句層 keyword bank：必然性框架（SSOT／權威住址／裁決陳述中的「唯一」與定義蘊涵的「必然」合規）、否定起手（糾正已文件化誤讀的「不是 A、而是 B」仍列可改，修法保留糾正語意）、歸因語氣（「承認」主詞為規範對實作的認可時合規）、泛用詞濫用（「視情況」「預設情況下」為固定搭配不計「情況」命中）、並列括號（嚴重度依條文可執行性分級）、集合命名用角色不內嵌數量（成員機制固定且緊鄰具名展開者可帶數字）各補一句邊界；新增「判準外交接」與「1-A 掃描的另兩份判準（非本清單成員）」兩則說明，前者釐清驚嘆號重複與檔頭三段式缺席不屬本清單，後者記錄隱含詞（歸文件撰寫明示性判準）與禁用詞豁免／zhtw 命中（歸語言約束判準）在 1-A 掃描中的合規邊界、正本仍在各自檔案。均為既有判準的射程邊界補充，不改變既有判準語意。

**Version**: 1.7.2 — 更正 0.92.0 那條刪除紀錄，並記下不恢復的理由。原文寫「職責已由其他機制覆蓋」而沒有指名接手者，另一句「任何 settings 都沒有註冊它們」把一個只在本專案驗過的事實寫成關於全體的宣稱——兩個下游消費端各有四處註冊，其中一個在刪除前五天還修過那支腳本，三方各找過一輪才確認那個「其他機制」不存在。改寫後照三選一走：寫出不適用的條件並標明作用域、寫明沒有接手機制、寫明沒有已知缺陷。同條的傳播語意一併修正——`--prune` 擋的是還沒裝的人，`pull` 保留本地獨有的檔，所以已安裝的消費端會同時持有刪除紀錄與那個檔，兩份互相否證而沒有訊號。**本次不恢復那兩個檔**：它們的主題與本 skill 的主題不符（共用庫裡其餘七支帶 `hooks/` 的 skill，hook 主題都與該 skill 一致，本 skill 是唯一的例外），而本 skill 的版號由不跑它們的一方持有，恢復會把「兩個作者、一個版號」固化——那正是這次落差的產生方式。要共用的話，正確的落點是另立一支由使用它們的一方持有版號的 skill；放進 `references/project-integration/` 不算解法，那個目錄排除於同步之外，等於同一份內容各自複製一份。

**Version**: 1.7.1 — `name-collections-by-role-not-count` 的章節編號段補射程限制：借自上游權威的序數命名（規格自己的層級名）不在此列，它與章節編號在掃描底下長得一樣而承載的是外部的名字。判別問句是「這個序數是本文件排出來的，還是外面既有的名字」。同段補上三十二處移除逐處查過、沒有一處因被引用而留下。

**Version**: 1.7.0 — 兩項。`name-collections-by-role-not-count` 補「章節編號是同一類、而且最容易漏」：判別問句多一個更省力的版本（這個數字是我寫的，還是排版產生的），排版產生的那些沒有人會 review 因為它們看起來像結構不像內容，處置是整批移除而不是重編。`parallel-parentheses-imply-a-uniform-relation` 補射程限制——它是條文層的檢查、對拆分點沒有預測力，一次實測的拆分點全部來自動作的時序，而括號最集中的一節關係統一且整節沒被拆。

**Version**: 1.6.0 — 四項來自一次四輪審查的補強。索引建立段補相對位置的失效條件（跨檔即失效、目標可能移動即失效；同檔緊鄰合法，判準比編號寬一階），並記下位置指涉的主要來源是修法自己——寫下的當刻位置一定是對的，所以掃描的觸發是改動不是完稿。集合命名段補散文端的載體：承擔指涉功能的計數沒有「刪掉」這個出口，修法是改成不自帶計數而不是改對數字，判別問句是這個數字由什麼決定。字句層 bank 新增「並列括號的關係檢查」（形態可掃、關係要逐個寫下來，一次實測六個括號被判讀出六種關係），並新增 principle 卡 `parallel-parentheses-imply-a-uniform-relation`。翻譯探針段補自稱可機械執行的條文要先關掉的兩個語法項（基數、否定的轄域），以及一個判讀限制——這一類的分歧不會進決策清單。
...
**Version**: 0.92.0 — 刪掉 `hooks/` 目錄下的 comment-qa-hook.py 與 worklog-format-check.py（本條於 1.7.2 改寫，原文的刪除理由沒有指名接手者、且把一個只在本專案驗過的事實寫成關於全體的宣稱）。**兩者的主題都不是本 skill 的主題，路徑也綁在另一個專案上**：comment-qa-hook 檢查 Dart / JavaScript / TypeScript 的事件處理函式與 Widget 有沒有完整的追溯註解，並指名一份事件驅動架構文件；worklog-format-check 掃某個工作日誌目錄底下表格內會讓 CLI 崩潰的字元。**在本專案不適用的條件**（本專案是一個 Hugo 靜態站）：沒有 Dart 原始碼、TypeScript 只有 Playwright 的設定與 spec 檔而該 hook 自己豁免測試檔、也沒有它指名的那份架構文件；工作日誌的目錄名與它寫死的不同。**沒有接手的機制**——emoji 在本專案由寫作規範加人工 grep 管，那是風格層，不是那個崩潰的緩解。**沒有已知缺陷**，還在跑它們的消費端不必因為這次刪除而停用。**作用域**：以上判定只涵蓋本專案，未查其他消費端。由一次「為什麼工作區有 hook-logs」的追查暴露：那個目錄底下的空資料夾正是這個 hook 的產物落點。**刪除的射程只到還沒裝的人**——`skill-sync push --prune` 讓遠端不再帶著它，而 `pull` 保留本地獨有的檔（預覽標 `local-only (preserved)`，拉取路徑沒有任何刪除呼叫），所以已安裝的消費端會留著檔案並同時收到這條宣告刪除的紀錄。同批修掉 0.90.0 那條版本紀錄裡的來源專案路徑——可攜性閘門反覆攔在同一處，而它只需要留下論點、拿掉路徑
