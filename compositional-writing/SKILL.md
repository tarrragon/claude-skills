...
**Version**: 0.79.0 — 合併本地 fork 與發佈庫（發佈庫 0.78.0）的雙向分歧：收入遠端 5 個新 principle 卡（decodable-from-text-already-read / no-fabricated-experience-or-attribution / readers-form-their-own-judgments / verify-by-recognition-not-recitation / writing-designs-the-reading-process）、word-choice-fits-concept-attributes 三種錯配擴為四種、translation-review 正向陳述改寫、writing-articles 一處偽標題改真標題（MD036）；同時攔截遠端本次同步帶入的用語回退（`language-constraints.md` 規則 2 禁用詞清單，「文件」「資料」「程式碼」「預設」四類正字被寫成對岸慣用寫法，共 25 處），跨 9 個檔案（SKILL.md / writing-articles.md / cadence-homogenization.md / write-for-readers-not-audiences.md / source-to-teaching-analysis.md / anecdotal-source-needs-mechanism-reconstruction.md / hyperbole-legitimacy-by-position-function.md / regional-terminology-alignment.md / ease-of-writing-vs-intent-alignment.md）逐處改回合規用字或整檔保留本地版本；保留本地 `scripts/portability-check.sh` 與 axis-named-by-proxy-not-mechanism.md 的 `portability-allow` 註解（遠端誤刪，本地腳本仍依賴此標記判斷豁免）；dry-run-guide.md 採遠端的相對路徑寫法（`./scripts/portability-check.sh`，較不綁定安裝路徑）。
