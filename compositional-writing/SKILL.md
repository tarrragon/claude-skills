...
- **本 skill（compositional-writing）** 提供每輪 frame 的字句層 keyword bank — Round 1-A 寫作規範 reviewer 必須跑：
  ...
  - **1-A 掃描的另兩份判準（非本清單成員）**：跑 1-A 全批掃描時，「隱含詞」歸文件撰寫明示性判準，合規邊界已載於 `.claude/rules/core/document-writing-style.md`〈撰寫前檢查清單〉；「優化」等禁用詞豁免範圍與 zhtw cross_strait／confusable 命中判專案 canonical 歸語言約束判準，合規邊界已載於 `.claude/rules/core/language-constraints.md` 規則 2。三者各自的正本不在本清單，語意與命中規則仍以各自正本為準，本節不重複載述條文內容
