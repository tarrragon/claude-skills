...
**正文裡的對象用它是什麼來命名，不用序數、計數或字母代號**：名字的職責跟程式變數相同——讀者只看到名字就知道它指什麼。「三支」「七步」「第三步」「第一級」「前提一」「A1 與 C」是作者規劃結構時的索引，讀者要手上有那份清單才解得開，而作者與同源 reviewer 都持有清單，所以這一類在審查裡沒有成本、只在從中段落進來的讀者身上有。**生成端就用內容名**：分組用它回答的問題、步驟用動作、分級用它的判準、並排的做法用描述；段標不帶序號，引用用節名或內容。判讀兩問：這個名字當變數名稱過不過得了程式碼審查；讀者從搜尋落在這一句，知不知道它指什麼。「這個集合不會再變」不構成保留序數的理由——上一條的損害要等成員增減，這一條在成員不變時就已經發生。序數名字密集的段落多半同時在寫作者的編排思路（各篇怎麼分組、兩篇怎麼分工），那些是維護資訊、不進正文。詳見 [ordinal-labels-are-the-authors-index](references/principles/ordinal-labels-are-the-authors-index.md)。
...
- **本 skill（compositional-writing）** 提供每輪 frame 的字句層 keyword bank — Round 1-A 寫作規範 reviewer 必須跑：
  ...
  - **序數、計數與字母代號當名字 grep**：`rg "[一二三四五六七八九十兩]支|[一二三四五六七八九十]步|第[一二三四五六七八九十][步支節級層章階]|前提[一二三四五]|兩個軸|另一個軸|這個軸"` 與 `rg "\b[A-E][0-9]?\b 那|\b[A-E][0-9]? 與 [A-E][0-9]?\b"` — 正文裡的實體用作者的索引當名字（「第一支的篇落在七步的第三步」「D 與 E 在這裡」），讀者要回頭翻才知道指什麼；判讀用變數名稱測試（當成程式變數名稱過不過得了審查）與落點測試（從中段落進來知不知道它指什麼），修法是用問題／動作／判準／描述命名、段標拿掉序號。**詞綴隨內容變、這條 pattern 非窮舉**：換一個分類先盤點那裡用的是支、步、級、節、層、階段還是別的。命中是候選——緊鄰清單且隨即逐一具名展開的計數、量測事實的數字、外部凍結編號、確實在描述位置而前一句剛列完成員的序數都合規；**「集合不會再變」不構成放行理由**。命中密集的段落順手問它是不是在寫作者的編排思路（各篇怎麼分組、兩篇怎麼分工、改哪一篇），那一類整段移出正文。見 [ordinal-labels-are-the-authors-index](references/principles/ordinal-labels-are-the-authors-index.md)
