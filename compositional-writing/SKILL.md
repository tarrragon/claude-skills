...
**拆完之後有一組新的檢查，對象是拆分自己造出來的交界**：一份文件拆成多檔之後多出一個原本不存在的位置——讀者在這裡離開一份檔案、進入另一份，或者不知道還有另一份可以進。**每一份單獨讀都是對的，所以逐檔的自審與逐檔的冷讀對這一類恆真**（一次實測：一份拆成八檔的文件通過三輪高階 reviewer 的九個 frame 之後，冷讀量測產出的七項判決一項都沒被攔下）。六條逐項檢查：兄弟檔互指要對稱（逐檔取檔頭的同目錄引用排成矩陣、看缺格）、每份要有位置座標而座標寫上一步與下一步不寫總數、層級不得混疊（子檔的步驟編號不可與全程編號指同一件事而各編一次號）、用詞的定義與使用不得分屬兩檔、內嵌於散文的計數改用不帶計數的分組名、**受管詞的指路寫在首次用點不寫在檔頭**（檔頭總括式的效果量得出來是零：八份檔頭都有那句話而三份仍判「沒說去哪查」，只把一個詞改成首次用點就地帶指路之後降為零份）。**切法會決定層級混疊會不會發生**——依角色切讓每一份從自己的第一步編號、於是同一件事在兩個層級各編一次，依步驟切不產生它，所以流程型內容的拆分軸優先選步驟。外移一段之後還有一個固有的擺盪（原處被讀成掏空、補回又被讀成堆砌），留在原處的殘餘要能自足回答「為什麼這一步值得慢慢做」，而自足的成本是一個具體實例、不是一串後果。詳見 [split-defects-land-between-the-files](references/principles/split-defects-land-between-the-files.md) 與 `references/structuring-with-solid.md` 的〈拆完之後的交界檢查〉。
