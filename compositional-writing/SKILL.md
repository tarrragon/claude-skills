...
上一節定的是整份稿件的定位；這一節定的是每一句的規格，寫的當下就用，不等審查。

描述一個操作的句子由四個槽位組成——**動作者**（人、腳本、工具、系統元件）、**動詞**（這個領域裡的操作：執行、轉換、比對、寫入）、**受詞**（動詞在這個領域裡能作用的對象）、**結果**（輸出或狀態變成什麼）。每個槽位填的都是這個領域裡實際存在的東西；量詞計數的是領域裡的單位。
...
右邊那一句在中文語法上完整、讀起來通順，而「執行」在這個領域裡作用的對象是指令、程式、命令——**沒有任何系統逐字執行東西**。「一個 X 都沒有 Y」在口語裡成立的前提是 X 是 Y 的計數單位（「一個字都沒說」成立，字是說話的單位），搬到「執行」上前提不成立，句子照樣通順，因為中文語法不檢查受詞跟動詞在領域裡合不合。讀者要做四步還原（認出慣用語 → 把字對回指令 → 把一個都沒有對回零個 → 重組），而左邊那一句直接就是終點。

判斷標準是**受詞是不是動詞在這個領域裡能作用的對象**：寫的當下逐句可判、不需要上下文。量詞槽位另有一條：**範圍在句內具名**——「任何指令」具名、單位在領域內，過；「一個字」具名、單位在領域外，不過；「徵兆完全一樣」範圍沒具名，不過，而且那篇文章的下一節給了分辨方法，「完全」跟內容矛盾。範圍寫在同一句或緊接的前一子句都算具名。**判定者不代填槽位**：單位要寫在句子裡，判定者從上下文推得出來不算——一次實測判定者替「完全一樣」補了「逐項相同的徵兆」這個單位然後放行，那正是規格要擋讀者做的動作換成審查者在做。判準是是非題「單位有沒有寫在句子裡」，寫成「推不推得出單位」它永遠通過。先前的「拿掉強調事實變不變」要有人先拿掉才知道，是後驗的，本節的判準取代它。同一個標準管三種變體：受詞填了領域外的名詞（「一個字都沒有執行」）、動作者填了指令片段（「`tr -d '\r'` 處理的是行尾」——被處理的是輸出裡的位元組，行尾轉換是處理的結果，「行尾」填在受詞槽位是錯的）、位置槽位填了指示詞（「字串比對在這裡會失敗」——「這裡」不是領域裡的位置）。

這一節放在生成入口、與字句層 bank 分開，理由是 bank 是反模式清單、給審查者掃已寫好的稿，而寫的人跟審的人是同一個模型時，審查端對這一類的偵測率是零——同一個模型覺得自然的句子，它寫出來、它審過去。生成器需要的是正向規格：寫每一句操作描述之前先填四個槽位，槽位填不出領域內的名詞就不成句。慣用語進不來，因為慣用語的受詞幾乎都在領域外。

段落層的順序跟著槽位走：讀者會看到什麼 → 這個工具做什麼 → 為什麼需要它（機制，系統元件指名）→ 不做的具體後果 → 副作用。動作者槽位填對之後段落順序通常自動修正——操作者視角的順序就是讀者的問題序。詳見 [command-fragment-as-subject-hides-the-actor](references/principles/command-fragment-as-subject-hides-the-actor.md) 與 [spoken-emphasis-is-silent-in-text](references/principles/spoken-emphasis-is-silent-in-text.md)。

## 示範指令的引數另有一條檢查（程式碼區塊適用）

上一節的判定單位是句子，而指令沒有主詞也沒有謂語，逐句填槽位的動作走到程式碼區塊就停下來。區塊裡的每一個字面值因此通過每一道句子層檢查。

示範指令有四個位置——工具、子命令、選項、引數——其中引數最需要寫：工具與子命令的名字讀者查得到，選項多半有 `--help` 列得出來，而引數的語意常常只有文章講得出來（路徑為什麼選在這裡、兩個引數哪個是來源、給不給這個引數會讓指令做不同的事），而且它是讀者唯一要動手改的位置。
...
**集合命名用角色、不內嵌數量**：標題要當穩定錨、就得先是純 fact —「核心七問」「成長六階段」「四大支柱」把成員數烤進名字、數量是成員清單的 derivation、加一問名稱先失真、所有複製過名稱的地方跟著過期。命名只承載角色與層級（核心問題 / 撞牆階段 / 支柱）、數量讓清單自己呈現；外部凍結品牌（SOLID 五原則 / OWASP Top 10）跟概念閾值（兩次門檻）的數字是 fact、可留。**同一個機制在散文裡的載體掃不到而數量更多**：承擔指涉功能的計數（「後三列」「三則」「共用兩節」）沒有「刪掉」這個出口，且它的產生源是結構改動——寫下的當刻數字是對的，加一列、拆一檔、搬走一節就當場過期，每一輪修法都在生產新的。修法是改成不自帶計數（把數字改對只買到一輪），判別問句是**這個數字由什麼決定**——由會被改動的集合的成員數決定就不安全，由模式數 / 階段數這類不隨結構增刪的東西決定就安全；掃描的觸發同樣是改動、不是完稿。**合規邊界**：成員由機制固定且緊鄰具名展開者可帶數字（如「六欄位」「三查」）；`##` 標題帶數字（如〈五種情境〉）須與表列數一致並註明例外成員，否則仍屬違規。詳見 [name-collections-by-role-not-count](references/principles/name-collections-by-role-not-count.md)。

**錨點標籤的自足義務隨引用點數上升**：檢索鍵位（title / 表格鍵欄 / 段標 / 欄位標籤）的精簡優先只在這個鍵**只有一個消費點**時成立。鍵一旦被別處逐字引用它就是錨點，而引用的形式是逐字比對——加字即斷開，所以每個引用點只交付短名本身、補完的責任全部落回讀者；短名省下的字數付一次，還原限定語的成本乘以讀者數與引用點數，而既有標籤要改時它與全部引用點必須同一次改、修復成本同樣乘上去。判準是兩個問句：**這個標籤裡的每個名詞，讀者知道是哪一個嗎；每個動詞，讀者知道主詞是誰嗎**。省略的限定語補回標籤本身、不補在鄰欄（讀者先讀標籤才決定要不要讀那一列）。偵測的入口是引用點數（`rg -F -l -- "<標籤>" <根目錄>`、命中兩檔以上就是錨點），對象是定義處的標籤清單而非全文段標——用段標當代理會量到模板重複（一次實測前幾名全是 `YAML`、`Problem` 這類模板欄位）。**這一類讀起來是完整的詞，所以查殘片的檢查（名詞頭、指涉閉合）全數放行、一致性掃描也無效**——壓縮零星發生，周圍都是對的。例外：表頭語境下已自足的通用欄位標籤（「觸發」「反應」）、領域內一跳查得到的正式術語。詳見 [compressed-label-cost-multiplies-with-citations](references/principles/compressed-label-cost-multiplies-with-citations.md)。
...
**不寫會取代讀者查證的東西（上一條的反向配對）**：特定實例的值與處理方式（某台機器、某個環境、某次安裝）寫進文字之後，就佔據了讀者原本要自己查的那個位置——他要的是「我這一台是什麼」而文字給了「有一台是什麼」，而**特定實例的值正好是他那一台會不一樣的那些**。**危害與正確性無關**：值可以完全正確、來源完全可靠，而它從來就不是關於他的系統的。判別問句是**讀者拿這一句去用，他是去查自己的系統，還是直接照抄**；修法是換成該去哪裡查，或在機制需要例子才講得清時用明顯的佔位值並寫明它是佔位的。不受約束的三類是機制、規範定義、公開介面的行為（在每個實例上相同，查證與照抄得到同一個答案）。**「標註未驗證」對這一類不夠**，差別在誰承擔預設——標註要讀者的自律才生效，不寫由結構保證；而照抄的人多半搜尋直達那一行、複製、離開，路徑上不經過那個標記。伴生判斷「需要實機才寫得了」多半是把「有了會更好」講成「需要」：主題拆成「問題的形狀與該去哪裡查」與「某個實例上的具體答案」兩半之後，前一半不必等硬體、後一半不該寫。與上一條是反向的一對——那條管該寫而沒寫，這條管不該寫而寫了，兩者都在稿件上看不出來。詳見 [written-value-replaces-the-readers-check](references/principles/written-value-replaces-the-readers-check.md)。
...
**句子層文法標準（全部書面產出適用）**：技術文章描述操作的每一句由四個槽位組成——動作者、動詞、受詞、結果——每個槽位填的是這個領域裡實際存在的東西，量詞計數的是領域裡的單位。判斷標準是**受詞是不是動詞在這個領域裡能作用的對象**：「執行」作用於指令，「一個字都沒有執行」的受詞「字」在領域外，句子語法完整而讀者要做四步還原；「處理的是行尾」的受詞「行尾」不是被處理的東西，被處理的是輸出裡的位元組。這條標準獨立於文章的定位、獨立於消費位（鍵位或段落位都適用），與行自足是兩個獨立的檢查——行自足管的是句子在脈絡外能不能還原命題，本條管的是每個槽位填的名詞在不在領域裡。它是生成端規格（見〈操作句先填槽位再成句〉），寫的當下逐句填槽位；審查端 grep 抓不到（領域外的名詞沒有固定關鍵詞），偵測靠冷讀或理解探針的「這個動作是誰做的」與「對什麼做的」兩欄。詳見 [command-fragment-as-subject-hides-the-actor](references/principles/command-fragment-as-subject-hides-the-actor.md) 與 [spoken-emphasis-is-silent-in-text](references/principles/spoken-emphasis-is-silent-in-text.md)。
...
- **本 skill（compositional-writing）** 提供每輪 frame 的字句層 keyword bank — Round 1-A 寫作規範 reviewer 必須跑：
  ...
  - **必然性框架 grep**：`rg "天生|與生俱來|本質就是|本來就是|必然|唯一|註定|理所當然"` — 把設計選擇講成自然法則、還原成條件性（物理 / 法律 / 數學事實除外）。**合規邊界**：SSOT／權威住址／裁決陳述中的「唯一」（如「唯一權威定義」「唯一標準指令」）與由機制定義直接蘊涵的「必然」（如「A 與 B 必然分屬兩個 commit」）合規；違規限把可變的設計選擇講成自然法則，改寫成條件句反而會失去 SSOT 語意
  ...
  - **歸因語氣 grep**：`rg "承認|暴露了|證明了失敗|被迫"` — 描述系統行為用「信號」「反映」「顯示」等中性觀測詞、避免「承認」「暴露」等責任歸因詞；「被迫」在描述外部強制約束時可保留。**合規邊界**：「承認」的主詞是規範／文件對實作的認可（非人對過失的認錯）時合規
  - **宣導語氣 grep**：`rg "你可能沒注意|你可能不知道|想像一下|把.{1,5}想成|跑得好好的|聽起來很|其實很簡單|說穿了就是|等於拆未爆彈|乾瞪眼|延遲引爆"` — 預設讀者無知或用情緒管理取代事實陳述；讀者是專業人士、直接描述情境與後果
  - **泛用詞濫用 grep**：`rg "坑|東西|搞|弄|處理一下|情況"` — 同一個泛用詞蓋過不同具體情境時、依情境換精確詞（意外 / 陷阱 / 出問題 / 發生狀況）；命中密集且各指不同事才算違規、真泛指 / 引號引用 / 輕度 hook 合規；「坑」另有地區偏移面（某些地區高頻、某些少用）。**合規邊界**：「視情況」「預設情況下」為固定搭配，不計入「情況」命中。見 [avoid-overused-generic-words](references/principles/avoid-overused-generic-words.md)
  ...
  - **教室形式三形態**（教學定位被讀成教室講課時的殘留，三個形態各有檢查方式）：
    - **形態一：情境故事開場**——判別是刪掉開場第一段之後讀者還知不知道這篇在講什麼。標題與 description 已經告訴他了，開場的職責是一句話交代適用範圍。改前：「本篇的情境來自一次實際排查⋯⋯」；改後：「這篇整理透過 Wi-Fi 連上 adb 之後，用來做遠端排查的指令與注意事項。」
    - **形態二：省略主詞受詞**——grep 抓不到（缺的字沒有關鍵詞），靠冷讀（2-B′）或理解探針。判別是每一句的操作主體讀者在只看這一段的情況下能不能還原。改前：「回答的卻是沒問的那個問題」；改後：「`dumpsys user` 的 `grep -i restriction` 會撈到 `Guest restrictions` 區塊裡的 `no_install_unknown_sources`，這是 guest 使用者型別的預設清單。」
    - **形態三：教室互動語句 grep**：`rg "該問誰|要問誰|容易搞錯|容易讀反|容易認錯|前面提到的|前面說的|先確認.*再|先讀懂.*再|注意這裡"`。修法是把互動句改成直述——「這裡容易讀反」改成引導看對的欄位並給出正確的讀法；「不能預設第一行就是有焦點的那一組」改成「跳過兩欄都是 `null` 的組，找到欄位有值的那一組」。command 中性陳述語句合規
    - **形態四：判定標準包裝成自問自答 grep**：`rg "問自己|問一個問題|然後問"`。改法是拆成條件與通過標準
    - **形態五：「問得出/問不出」替代指標 grep**：`rg "問得出|問不出來|問不出"`。改法是寫出指標名稱與通過標準
    - 根因與判讀徵兆見 [teaching-is-not-lecturing](references/principles/teaching-is-not-lecturing.md)
  - **補充資訊歸位**（先備知識與環境陷阱有沒有跟在所屬的指令旁邊）——grep 抓不到，判別問句是「讀者在讀到哪個位置時會需要這段資訊」。遊離的形態：參數的理由獨立成段而讀者掃表格時跳過它、環境陷阱跟在無關的段落後面。修法是搬到所屬的指令或概念旁邊——表格欄位裡、或緊跟指令的說明段。改前：`-G 16M` 的放大理由寫在表格下方一段散文裡；改後：搬進表格的用途欄。見 [supplementary-knowledge-follows-its-parent-concept](references/principles/supplementary-knowledge-follows-its-parent-concept.md)
  ...
  - **指令引數未說明的檢查**：有詞形可掃的三類——`rg -n '^\s*[a-z].* /[a-z][a-zA-Z0-9_./-]*'`（指令裡的絕對路徑）、`rg -n '^\s*[a-z].* [A-Z][A-Z_]{3,}'`（全大寫的常數引數）、`rg -n '<[a-z_]+>'`（佔位符，用來對照哪些位置已經被解釋過）。**命中是候選、而更重要的是漏網的那些**：小寫識別碼形式的引數（`secure`、`windows`、`allow`）沒有穩定詞形，grep 抓不到，要逐條指令過判準問句。判定看那個位置的說明在不在全文裡，不看指令本身寫得清不清楚——**區塊裡的字面值讀起來永遠是完整的**，所以逐句自審與同源冷讀對這一類一律回報通過。見 [literal-operand-hides-the-change-point](references/principles/literal-operand-hides-the-change-point.md)
  - **並列括號的關係檢查**：沒有關鍵詞可掃、形態可掃——`rg -n "^\s*[-*0-9].*（.+）\s*$"` 找出以括號收尾的條列項，連續三個以上就把整組抽出來並排，逐個寫下它與前面那個詞的關係（解釋 / 定義 / 範例 / 驗證方法 / 紀錄內容 / 度量）。**版面上的同構本身是一個宣告**，讀者依它去解讀每一個括號，各裝不同東西時他找不到那條統一規則就逐個猜——一次實測六個括號被探針判讀出六種關係，其中「形態數（簽核人與日期）」被讀成定義而照此執行的人會去數簽核人。每一個括號單獨看都通順，所以逐句自審過得去、要整組並排才看得見。修法是換成表格的顯性對映（讓表頭說出那個關係），或在括號裡加關係詞。條文被宣告為可執行時零容忍。**合規邊界**：嚴重度依條文可執行性分級——可執行條文零容忍列可改；目錄型列表與純說明性條列降為 Info；AUQ label／description 等規格字面的括號（內容為欄位規格非解釋）不計。見 [parallel-parentheses-imply-a-uniform-relation](references/principles/parallel-parentheses-imply-a-uniform-relation.md)
  ...
  - **命中數不是涵蓋率**：一條回傳大量命中的 pattern 讀起來像「這一類有在管」，於是「這個體裁的這一類還有沒有別的形態」不會被問——零命中至少還會讓人懷疑 pattern 寫錯了。跑完一批修正之後回頭比對命中數：修了幾十處而數字幾乎沒動時，修的東西與量的東西不是同一批。這個對照免費（改動前後各跑一次），而它是涵蓋缺口最早的訊號。**掃描指令本身也要驗**——同一個 session 內三次因為指令而非內容漏報：zsh 的字串沒斷行、`uniq -c` 的輸出被誤讀、以及要求前後脈絡的 regex 略過了靠近行首行尾的命中。回報 clean 之前，先拿一個已知會命中的字串餵給同一條指令。 **這三種裡最後一種有個可執行的修法，而它背後是一組相反的失效方向**：換行與儲存格分隔符要**替換成單一空白**，不要刪除、也不要拿它們當切段的邊界——拿來切段會讓跨行或跨格的命中漏報，直接刪除會把相鄰的字接成原文沒有的字串而生出假命中，兩者都不報錯。取前後脈絡的掃描因此寫成不限長度的形式（`.{0,20}` 而不是 `.{20}`），並在回報零行時先看計數是不是也為零——**計數非零而脈絡零行，零的是切段不是內容**。
