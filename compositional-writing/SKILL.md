...
**會讓稿件被讀成手冊的裝置**（掃裝置比掃語氣可靠）：表徵對映清單（「症狀：成因」）、檢查順序的指示、配比與參數數字、順序關鍵加上顛倒的後果、可執行的操作判斷標準、具體器具與工具作法、**序列連接詞（先、之後、最後）**。最後一項最容易漏——把步驟編號換成它只改掉一半。
...
判斷標準一句話，逐條指令可執行：**讀者要把這條指令用在自己的環境上，他得改哪幾個位置；每一個要改的位置，文章有沒有說過它填的是什麼。** 答「不用改任何位置」的指令不適用。
...
五種形態、修法與掃描指令在 [literal-operand-hides-the-change-point](references/principles/literal-operand-hides-the-change-point.md)。 區塊怎麼排、註解裝什麼、何時改用參數表，連同 before / after 走查與檢查清單在 [demonstrating-commands](references/demonstrating-commands.md)。
...
**引用錨點用語意標題、不用位置編號**：引用另一個章節 / 階段 / 條列項時寫「見核心問題」、不寫「見 Stage 3」— 編號是結構排列的 derivation、結構重排時引用句字面完好、語意 silent 指向錯的內容（比 broken link 難偵測：連結斷掉會報錯、編號錯位會成功解析到錯的東西）。對應要求是每個結構單位的標題要承載核心意義（「Stage 3：核心問題」、編號只作排序前綴）、引用取語意半邊；發布方凍結的編號（RFC 段號 / 法條）是 fact、可引用。**相對位置（末段 / 下段 / 上節 / 下方）是同構的另一種 derivation 而判斷標準寬一階**——同檔緊鄰的合法且常比具名好讀，失效條件有兩個、各自獨立：跨檔即失效（讀者手上的「上節」是他正在讀的那一份的）、目標可能移動即失效（不報錯，成功解析到現在住在那個位置的東西）。由第二個推出**位置指涉的主要來源是修法自己**（寫下的當刻位置一定是對的），所以掃描的觸發是改動、不是完稿；定址型產物（地址表 / 索引 / 路由表）一律具名、不看距離。詳見 [reference-by-semantic-title-not-number](references/principles/reference-by-semantic-title-not-number.md)。
...
**正文裡的對象用它是什麼來命名，不用序數、計數或字母代號**：名字的職責跟程式變數相同——讀者只看到名字就知道它指什麼。「三支」「七步」「第三步」「第一級」「前提一」「A1 與 C」是作者規劃結構時的索引，讀者要手上有那份清單才解得開，而作者與同源 reviewer 都持有清單，所以這一類在審查裡沒有成本、只在從中段落進來的讀者身上有。**生成端就用內容名**：分組用它回答的問題、步驟用動作、分級用它的判斷標準、並排的做法用描述；段標不帶序號，引用用節名或內容。判讀兩問：這個名字當變數名稱過不過得了程式碼審查；讀者從搜尋落在這一句，知不知道它指什麼。「這個集合不會再變」不構成保留序數的理由——上一條的損害要等成員增減，這一條在成員不變時就已經發生。序數名字密集的段落多半同時在寫作者的編排思路（各篇怎麼分組、兩篇怎麼分工），那些是維護資訊、不進正文。詳見 [ordinal-labels-are-the-authors-index](references/principles/ordinal-labels-are-the-authors-index.md)。

**摘要另一份內容時寫出成員，不寫計數**：在介紹另一份讀者還沒讀的內容的位置（文章結尾指向下一篇、`description`、目錄頁交付欄、閱讀路線說明），「四個追問、三種寫法、四類邊界」只交付清單的長度——寫的人手上有那份內容所以數字對他是完整索引，讀者手上沒有。寫的當下把成員寫出來，數字通常可以拿掉；成員太多時寫共同的性質。判讀問：沒讀過那一篇的人，讀完這個片語知不知道成員是什麼。詳見 [counted-summary-delivers-only-the-length](references/principles/counted-summary-delivers-only-the-length.md)。

**錨點標籤的自足義務隨引用點數上升**：檢索鍵位（title / 表格鍵欄 / 段標 / 欄位標籤）的精簡優先只在這個鍵**只有一個消費點**時成立。鍵一旦被別處逐字引用它就是錨點，而引用的形式是逐字比對——加字即斷開，所以每個引用點只交付短名本身、補完的責任全部落回讀者；短名省下的字數付一次，還原限定語的成本乘以讀者數與引用點數，而既有標籤要改時它與全部引用點必須同一次改、修復成本同樣乘上去。判斷標準是兩個問句：**這個標籤裡的每個名詞，讀者知道是哪一個嗎；每個動詞，讀者知道主詞是誰嗎**。省略的限定語補回標籤本身、不補在鄰欄（讀者先讀標籤才決定要不要讀那一列）。偵測的入口是引用點數（`rg -F -l -- "<標籤>" <根目錄>`、命中兩檔以上就是錨點），對象是定義處的標籤清單而非全文段標——用段標當代理會量到模板重複（一次實測前幾名全是 `YAML`、`Problem` 這類模板欄位）。**這一類讀起來是完整的詞，所以查殘片的檢查（名詞頭、指涉閉合）全數放行、一致性掃描也無效**——壓縮零星發生，周圍都是對的。例外：表頭語境下已自足的通用欄位標籤（「觸發」「反應」）、領域內一跳查得到的正式術語。詳見 [compressed-label-cost-multiplies-with-citations](references/principles/compressed-label-cost-multiplies-with-citations.md)。
...
**並排多種做法時，比較的軸要落在讀者接下來要交出的那份產出上**：軸的來源有兩個——那幾種做法彼此的差異（回的筆數、花的時間、取得到的欄位），或讀者讀完要交出的那份產出（他正在設計的資料結構、介面、型別）。取自做法差異的每一欄都正確、都可實測、主詞都寫清楚了，而讀者讀完不知道自己該怎麼辦，因為他要動手改的那份產出不在任何一欄裡。**把軸寫成「這個做法假設了什麼」仍然不算換到讀者那一側**——主詞還是做法；換過來的問法是「我要設計這個東西，某一項要不要唯一、允不允許缺」，而那一個決定同時決定幾種做法各自成不成立。既有檢查全數放行：軸的數量與種類數對得上（缺軸那一條的先兆不觸發），名字與機制也都對，錯在整組軸選在另一個平面上。判讀逐欄唸標題問主詞是誰，或問**讀者讀完這一欄會去改什麼**——改這段文字的降為格內內容，改他正在設計的那份產出的留作欄並排在前面。修法是先寫下讀者要交出的那份產出再往回列軸；原本的軸不刪、降成格。**跨篇的零件要在某一處並排，而那一處是新的內容**——互相連結不產生並排，讀者一次仍然只看得到一種。詳見 [comparison-axis-comes-from-the-readers-decision](references/principles/comparison-axis-comes-from-the-readers-decision.md)。**適用邊界**：那張卡出自單一內容集合的一次判定，而它的觸發條件靠「多數教材的讀者以工作為目的」這個未量測的預設落地——套到新的集合之前先自己答一次「這裡的讀者真正要交出的是哪一份產出」，**而答案要從那個集合已經宣告的定位取、不是當下猜**，並寫進審查紀錄（不留痕跡的話這一步成本是零，於是會被跳過）。判別不適用的軸是**那份產出的去留**而不是它的型別：通過評分就丟掉的作業不適用，會被別人接手維護的產出適用——**同樣是一段程式，這兩者不同**。判別的入口是「本篇在比較什麼」而不是「有幾欄」，排成散文的比較同樣在射程內。判讀表也接不住三種形態裡的第三種（零件散在各篇、根本沒有表可以逐欄唸），那一種直接走修法的第三條。
...
- **本 skill（compositional-writing）** 提供每輪 frame 的字句層 keyword bank — Round 1-A 寫作規範 reviewer 必須跑：
  ...
  - **計數式摘要（無穩定 pattern，按位置逐句讀）**：在介紹另一份內容的位置——文章結尾指向下一篇的句子、`description`、目錄頁的交付欄、閱讀路線的說明欄——逐一找「數字＋類別名」的片語（「四個追問」「三種寫法」「四類邊界」「上線前的三件事」），問沒讀過那一篇的人知不知道成員是什麼。成員緊鄰數字寫出來的合規（「自連接、視窗函數與相關子查詢三種寫法」），量測事實合規（「差三個量級」），只有數字加類別名的違規。**grep 分不出兩者**，`rg "^description:.*[一二三四五六七八九十兩]+ ?(個|種|類|項|條)"` 只用來縮小要讀的範圍。「位置與集合指涉」那條管回指已讀的集合（其他三篇），碰不到這一類。見 [counted-summary-delivers-only-the-length](references/principles/counted-summary-delivers-only-the-length.md)
  - **縮寫術語 grep**：`rg "判準"` — 「判準」是「判斷標準」的縮寫，詞面只在科學哲學與教育評量有穩定所指，工程讀者讀不出來；一律寫「判斷標準」，狀態義寫「X 條件」（停止條件、完成條件）。全站替換過兩次仍回流，所以 `content/` 由 lint 規則 `TERM-abbreviation` 以 error 擋下；**skill 目錄不跑 lint，這一類要手動掃**。討論這個詞本身時加「」或行內程式碼。見 [term-probe-measures-register-not-invention](references/principles/term-probe-measures-register-not-invention.md)
  - **序數、計數與字母代號當名字 grep**：`rg "[一二三四五六七八九十兩]支|[一二三四五六七八九十]步|第[一二三四五六七八九十][步支節級層章階種類條項]|前提[一二三四五]|兩個軸|另一個軸|這個軸"` 與 `rg "\b[A-E][0-9]?\b 那|\b[A-E][0-9]? 與 [A-E][0-9]?\b"` — 正文裡的實體用作者的索引當名字（「第一支的篇落在七步的第三步」「D 與 E 在這裡」），讀者要回頭翻才知道指什麼；判讀用變數名稱測試（當成程式變數名稱過不過得了審查）與落點測試（從中段落進來知不知道它指什麼），修法是用問題／動作／判斷標準／描述命名、段標拿掉序號。**詞綴隨內容變、這條 pattern 非窮舉**：換一個分類先盤點那裡用的是支、步、級、節、層、階段還是別的。命中是候選——緊鄰清單且隨即逐一具名展開的計數、量測事實的數字、外部凍結編號、確實在描述位置而前一句剛列完成員的序數都合規；**「集合不會再變」不構成放行理由**。命中密集的段落順手問它是不是在寫作者的編排思路（各篇怎麼分組、兩篇怎麼分工、改哪一篇），那一類整段移出正文。見 [ordinal-labels-are-the-authors-index](references/principles/ordinal-labels-are-the-authors-index.md)
  ...
  - **指令引數未說明的檢查**：有詞形可掃的三類——`rg -n '^\s*[a-z].* /[a-z][a-zA-Z0-9_./-]*'`（指令裡的絕對路徑）、`rg -n '^\s*[a-z].* [A-Z][A-Z_]{3,}'`（全大寫的常數引數）、`rg -n '<[a-z_]+>'`（佔位符，用來對照哪些位置已經被解釋過）。**命中是候選、而更重要的是漏網的那些**：小寫識別碼形式的引數（`secure`、`windows`、`allow`）沒有穩定詞形，grep 抓不到，要逐條指令過判斷標準問句。判定看那個位置的說明在不在全文裡，不看指令本身寫得清不清楚——**區塊裡的字面值讀起來永遠是完整的**，所以逐句自審與同源冷讀對這一類一律回報通過。見 [literal-operand-hides-the-change-point](references/principles/literal-operand-hides-the-change-point.md)
  ...
  - **判斷標準外交接**：驚嘆號等標點強度重複、逐字重複兩處歸「同一件事兩形」（結構層一致性判斷標準，非本清單類別）；skill / reference 檔頭缺三段式（何時讀／同目錄／溯源）歸結構約設定判斷標準（self-application sweep 的 self-grep 對象）。兩者字面上像字句層問題、但本清單無對應類別，不列為本清單 finding
  - **1-A 掃描的另兩份判斷標準（非本清單成員）**：跑 1-A 全批掃描時，「隱含詞」歸文件撰寫明示性判斷標準，合規邊界已載於該專案的文件撰寫明示性規則（其撰寫前檢查清單一節）；「優化」等禁用詞豁免範圍與 zhtw cross_strait／confusable 命中判專案 canonical 歸語言約束判斷標準，合規邊界已載於該專案的語言約束規則（禁用詞那一條）。本清單不重複載述條文內容，語意與命中規則以各自正本為準
...
└── references/
    ...
    ├── demonstrating-commands.md         # 情境 5f：示範指令（區塊內容、註解分工、並列對照、參數表判斷標準）
