...
>
> **來源**：methodology 骨架整合自 `tarrragon/blog` 的 `tech_writing_structure.md`（2026-04-17）；與 compositional-writing 核心原則的映射由本檔補充。
...
寫或改一篇教材之前，過 [teaching-article-context](principles/teaching-article-context.md) 的三項檢查，它們決定這一篇有沒有脈絡：

1. **這一篇在序列裡的位置**——說得出讀者手上已經有什麼、這一篇交付什麼、刻意不碰什麼。深度看第一項，不看題材的表面複雜度。檢查是把那三句話實際寫下來，寫不出來的那一句就是還沒定位好的地方——遮住 `description` 只看標題是另一個檢查，測的是 `description` 有沒有增量，通過條件相反，不要混用。
2. **論證用得到的材料就地寫進去**——承重術語當場定義、承重機制寫進正文、每一句的指涉能從已讀文字解出來。這三項作者看不見，成因是熟悉度。檢查是用一支列出全篇高頻實詞的工具拿到候選，逐個回全文查有沒有一處說過它指什麼；主題詞必然高頻而它不是漏定義的那一個，濾的方式是問「拿掉這個詞的定義之後，那幾句還成不成立」。
...
可能觸發 API 相容性問題。Kotlin 2.2 把這個從 warning 提升為 error。
...
#### 分析型文章的開頭：定位問題先行、不放敘事或寫作動機

分析型文章（財報判讀、產業比較、技術評估）的開頭承擔一件事：讓讀者知道「為什麼值得分析這個對象、判斷它的錨點是什麼」。第一段直接進入定位問題——這個對象在什麼結構位置、它的什麼特徵讓它值得被拿出來判讀——而不是先講一段對象的來歷故事。這是規則二「商業邏輯先於 CASE」在文章開頭層的具體形式：定位問題就是分析文章的商業邏輯層。

兩種常見的開頭失焦：

- **敘事性引言**：用創辦人故事、公司沿革、產品發展史當開場。這類敘事對「認識這個對象」有用、對「判讀這個對象」沒用——分析文章的讀者要的是判斷錨點，不是背景故事。背景資料該在判讀需要時就地引用，不是鋪在開頭當情境暖場。
- **寫作動機框架**：在正文寫「我們為什麼要分析這個對象」「這篇要補上某個數字的佐證」。寫作動機是編輯層的資訊（作者為什麼寫這篇），不是內容層的資訊（讀者能帶走什麼）。讀者不在乎這篇為什麼被寫出來；把動機寫進正文是把編輯決策洩漏給讀者。動機留給自己的規劃，正文只放判讀本身。

自檢：把開頭第一段的來歷句（「這個對象成立於 / 起家於 / 原本是」）與動機句（「我們 / 本文之所以分析」）都拿掉之後，剩下的還能不能讓讀者知道判讀錨點。不能，代表開頭還沒進入定位問題。
...
**位置決定性質、不是「有沒有摘要」**：放文首的摘要（一段 abstract、文件的 description 欄位）服務「讀者決定要不要讀、先建全局框架」、在讀者**還沒讀正文時**提供導航；放文尾的重述在讀者**讀完之後**把已知內容再講一次、無導航功能。同一句「本文講 X 的三個取捨」放文首是合理摘要、放文尾是重述補丁。

**例外**：跨章模組 / 長篇的**導覽型結尾**（串連各章、給下一步路由、列模組地圖）傳遞「結構關係」這個正文沒有的新資訊、不是重述、保留。真實內容裡最常見的是「一段重述 + 一句路由」的混合型 —— 修法是外科式：切重述段、留路由句、別整段刪。
...
#### 套樣板之前先定軸：這幾個選項要按什麼並排

上面那個樣板的每一區塊是一個**做法**，每一個欄位（機制 / 理由 / 適合 / 代價）的主詞也是那個做法。這個配置在讀者要交出的產出就是「一段程式」的時候是對的，而多數教材的讀者要交的是另一種產出——一份資料結構的設計、一個介面的形狀、一組型別的選擇。

軸的來源有兩個，只有一個對得上讀者：

| 軸的來源                                                       | 寫出來長什麼樣           | 讀者讀完會去改 |
| -------------------------------------------------------------- | ------------------------ | -------------- |
| 那幾種做法彼此的差異（回的筆數、花的時間、取得到的欄位）       | 每個做法一區塊，如上樣板 | 這段文字本身   |
| 讀者讀完要交出的那份產出（結構、型別、約束、介面）           | 每個設計決定一區塊       | 他那份產出   |

第一種的每一欄都正確、都可以實測、主詞都寫清楚了，而讀者讀完不知道自己該怎麼辦——**他要動手改的那份產出不在任何一欄裡**。把欄位改寫成「這個做法假設了什麼」不算換過來：主詞還是做法。

換過來之後同一批材料的組織是這樣：一個設計決定當一個區塊（「這一項要不要唯一」），區塊裡寫這個決定的兩種走法各自讓哪幾個做法成立、哪幾個失效。原本按做法寫的那幾格不刪，降級成格內的材料。

**判讀動作**：逐欄唸出標題問主詞是誰；或問讀者讀完這一欄會去改什麼。兩種軸都寫得出來的時候，設計決定那一種排在前面，做法差異是它的材料。

**跨篇的零件要在某一處並排，而那一處是新的內容。** 幾種做法各自住在幾篇裡、篇與篇互相連結，不產生並排——讀者一次仍然只看得到一種，而取捨只有在並排的時候才存在。這一段屬於內容壓力的就地展開，不屬於換成連結那個出口。
...
| 一段「為什麼要這樣做」離它解釋的指令超過一段               | 讀者在讀到哪個位置時會需要這段資訊？（[supplementary-knowledge-follows-its-parent-concept](principles/supplementary-knowledge-follows-its-parent-concept.md)）                             |
...
- [ ] 開頭語氣：把「你」換成「我們」後語句仍自然嗎？（彆扭 = 恐嚇式、改分享式）
...
**Version**: 0.9.0 — 從工具 opinion 文章 review 回流：(1) 輪 9 reader-sim 加第三 lens「meta 資訊 vs 內容」— 涵蓋 meta-commentary 殘留（AI 推理過程外露）+ 主題偏移（prompt context 系統知識洩漏）兩個 design gap；(2) 提交自檢清單加 3 個生成端自問句（恐嚇式 hook「你→我們」測試 / meta 刪除測試 / 歸因語氣判斷）。生成端自問句成本低於審查端 grep 擴充、且 4 個 design gap 中 meta-commentary 和 topic-drift 沒有穩定通用關鍵詞不適合 grep。
