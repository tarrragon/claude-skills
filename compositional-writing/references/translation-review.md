...
| 8. 量級檢查     | 譯文強度跟原文同級嗎？             | 定位強度詞、量級對位、跑鄰詞存在測試         |
...
可操作判準是**鄰詞存在測試**：目標語言存在一個量級更強的專詞、而原文沒有選用它、代表原文刻意停在較低量級。實例：某產品登入成功頁的英文標語「Build something great」、日文在地化「素晴らしいものを作りましょう」量級對位（素晴らしい 對 great、同為日常高強度讚美）；把日文句再交給 AI 中譯、得到「讓我們製造一點奇蹟吧」——日文有「奇跡」這個常用專詞、日文版沒選用它、中譯選「奇蹟」等於替上游做了一個它明確沒做的選擇。升格的來源是文體先驗：孤立短句、無上下文時、轉換者往目標文體的典型樣貌滑（中文行銷語「創造奇蹟」比「做出很棒的東西」更像 slogan）、翻譯默默變成文案再創作。
...
同一個「奇蹟」、在中譯一句日文標語時是超譯、在替自家產品寫中文標語時是誇飾修辭的正當運用。分辨能力比禁令有用：把三列混在一起、會在保真情境放行超譯、也會在創作情境誤殺有效的標語。未宣告目的的轉換、預設以保真為判準。鏈式轉換（翻譯的翻譯）漂移逐段累積、查證要回最上游的語言版本比對。量級命中是候選、判定在語意層：明示的意譯、宣告過的再創作、目標語言慣例要求的語氣調整（敬語、邀請式收尾）都合規。詳見 [rewrite-preserves-claim-intensity](principles/rewrite-preserves-claim-intensity.md)。
...
- [ ] 譯文強度詞跟原文同級；鄰詞存在測試通過（原文迴避的更強專詞、譯文也沒用）；命中先做語意判定再定違規
