...
- **沒有**：先不要修文字，**重新檢討寫它的動機**。動機是「說明這裡在做什麼」——技術面的資訊多半已由程式碼、型別與命名承載，回到查詢成本判準（唸出型別名稱、還剩什麼資訊）；動機是「怕有人改壞它」——走「動機先於文字」節的防護分岔（先問約束能不能消除、再交給會發聲的機制）。
...
判準是**這段資訊有沒有對應的斷言**：把型別名稱唸出來、說出還剩哪幾個字；剩下的問有沒有一個會紅的斷言撐得住（二元性掛在斷言存不存在，不掛在造得出句子）；來源在專案之外（法規、稽核要求、外部契約、當初的取捨）的才是註解該留的。後兩者可以同時成立——測試守行為、註解留出處。
