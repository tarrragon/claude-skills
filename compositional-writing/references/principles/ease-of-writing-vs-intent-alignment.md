...
> **角色**：本卡是 `compositional-writing` 的支撐型原則（principle）、被 SKILL.md 第 6 原則「Multi-pass Review」輪 2 引用、被 `references/writing-articles.md` / `writing-code-comments.md` / `writing-prompts.md` / `writing-documents.md` 在「對意圖」自檢段引用。
>
...
- 寫的時候的決策已經影響了周邊程式碼（要 refactor 一處要連帶改五處）
...
第 1 次 happy path 過了、覺得對。但 happy path 過 = 子集裡有命中、不證明 stream 全集對齊。第 1 次成功是低資訊量訊號。
...
判讀：寫之前自問「這程式碼三個月後會不會有人讀」 — 否 → 本原則可放寬；是 → 本原則嚴格適用。

---

## 跟其他抽象層原則的關係

本卡是「寫程式 / 寫文字 / 命名」當下選哪條路的共同上位原則 — 多個具體層原則（最小範圍、單一真實來源、外部組件邊界）都是「**便利 vs 正確性的取捨**」在不同情境的具體展現。

在 compositional-writing 內部、本卡是 multi-pass review 輪 2「對意圖」的核心判準 — 每段文字寫完後、用本卡的反相關檢查「我寫的順、是不是因為我選了便利但偏離意圖的寫法」。

兩個具體展現：

- [case-citation-three-part-structure](./case-citation-three-part-structure.md)：「對應 [case]」當段首最順、不寫概念定義最省字、便利跟意圖對齊反向
- [agent-team-context-isolation](./agent-team-context-isolation.md)：單一 reviewer 處理多維度最便利（不用 spawn / coordinate）、但意圖（深度 review）失準
