...
| 要求               | 正確範例                       | 錯誤範例              | 原因                                    |
| ------------------ | ------------------------------ | --------------------- | --------------------------------------- |
| 分層結構可拆解     | `v1.2.0-wave03-021.4`             | `ticket_74_sub7`      | 前者可用分隔符拆成版本/wave/序號/子序號 |
...
欄位集合是一個關於「會有哪些結果」的宣稱，而設計的人在列格式、不覺得自己在斷言什麼。**每一個沒有格子的結果，都被那張表宣告為不會發生**，而填表人遇到它時只有兩條路：改寫成填得下的形狀，或跳過那一列。

```yaml
# 錯誤：結果欄只容得下「有問題」這個敘事
result: "發現問題 → 已修正"     # 那「本來就對」的項目填什麼
```
...
實測有執行者把「初稿即為現況」寫成「發現後已修正」——那句話的每個字都對得上某個真實，唯一假的是它隱含了一次不存在的修正動作，而那正是欄位要求他提供的東西。**兩種失真的方向相反**（多一筆假的／少一筆真的），所以彙總時抵銷不了也看不出來。下游的代價是那個數字常常是後續判斷的分母。

**檢查方式**：逐欄列舉它可能的值，問有沒有一種真實情形落在全部的值之外。落在外面的就是會被改寫或被跳過的。**修法是補值，不是補說明**——填表的人選的是清單裡有的那一個。

同一條紀律用在報告的結論欄上，值有兩個：

| 值           | 例                                     | 涵蓋數被修正時                   |
| ------------ | -------------------------------------- | -------------------------------- |
| 正面命中     | 「這一題的三步可純字面完成」           | 不受影響                         |
| **窮舉宣稱** | 「其餘二十六題判否」「這一類只有兩道」 | **要跟著動** |

不分類的報告在涵蓋數被修正時只剩兩個極端可選：整份重驗，或整份照用。一次實測有了分類之後，那次修正的代價是重驗兩項而不是七項。

判讀與完整實例見 [field-shape-induces-false-reporting](principles/field-shape-induces-false-reporting.md)。
...
| `when`       | 何時啟動 / 何時截止 | `"v1.2.0 發佈前；依賴 wave03-021 完成後啟動"`                    | 產品行為時序     | 寫「當用戶點擊...系統會...」，描述產品行為而非 ticket 啟動條件   | 條件可被程式或人工驗證         |
